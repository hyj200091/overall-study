<template>
	...
	<view style="height: 100vh;" :class="nightStatus ? 'nightTheme' : ''">
		...
		<!-- 歌曲图片 -->
		<view class="flex align-center justify-center" style="height: 440rpx;">
			<image :src="musicCover" lazy-load style="border-radius: 35rpx;box-shadow: 0 2rpx 6rpx 0;height: 420rpx;"></image>
		</view>
		...
		<view>
			<view class="flex align-center justify-center" style="padding-top: 60rpx;">
				<view class="mr-3" @tap="PreOrNext('pre')"><my-icon iconId="icon-shangyishou" iconSize="85"></my-icon></view>
				<view class="mx-5" @tap="PlayOrPause"><my-icon :iconId="playStatus ? 'icon-bofang' : 'icon-ziyuan'" iconSize="80"></my-icon></view>
				<view class="ml-2" @tap="PreOrNext('next')"><my-icon iconId="icon-xiayishou" iconSize="85"></my-icon></view>
			</view>
		...
		<!-- 歌手具体信息 -->
		<view class="fixed-bottom shadow p-2 animated fadeInUp" style="height: 260rpx;border-radius: 30rpx;z-index: 0;" v-if="!listStatus">
			<view class="flex justify-between">
				<view>
					<view>
						<text class="font-weight-bold">歌曲：</text>
						<text class="font-weight-bold">{{ audioName }}</text>
					</view>
					<view>
						<text class="font-weight-bold">歌手：</text>
						<text class="font-weight-bold">{{ singerName }}</text>
					</view>
				</view>
				...
				<block v-for="(item, index) in audioList" :key="item.id">
					<view class="flex align-center font px-2" style="height: 85rpx;" hover-class="bg-light" @tap="selectPlay(item.id)">
						...
						<view class="flex-1 ml-3 flex align-center">
							...
							<my-icon iconId="icon-bofang" iconSize="40"></my-icon>
						</view>
					</view>
				</block>
			</scroll-view>
