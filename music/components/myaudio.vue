	...
	<view class="fixed-bottom rounded mx-2 mb-1" style="height: 160rpx; background-color: #d1ccc0;opacity: .9;" @tap="toDetailPage">
		...
		<view class="flex align-center justify-center font" style="color: #7a8388;height: 65rpx;">
			...
			<view style="width: 500rpx;">
				<slider block-size="16" activeColor="#e48267" backgroundColor="#eef2f3"
				 :max="durationTime" :value="currentTime"
		...
		<view class="flex align-center justify-between mx-2" style="height: 95rpx;">
			...
			<view class="flex align-center" @tap.stop>
				...
				<my-icon :iconId="playStatus ? 'icon-bofang' : 'icon-ziyuan'" iconSize="75" class="mx-2" @my-click="PlayOrPause('pre')"></my-icon>
				<view class="animated" hover-class="pulse">
...
<script>
	...
	export default {
		...
		computed:{
			// 得到歌曲的播放与暂停标识， 当前歌曲标识，音频总时长，音频播放时刻
			...mapState({
				playStatus: ({
					audio
				}) => audio.playStatus,
